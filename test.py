...
def get_pred_dir(model, data_root = './datasets/', save_path = 'preds/',img_size = 384,methods = 'DUT-O+DUTS+ECSSD+HKU-IS+PASCAL-S+SOD'):
    batch_size = 1
...
def test(args):
    print('Starting test.')
    model = DUP_MCRNet(dim=64,img_size=args.img_size,method=args.method,mode='test')
    model.cuda()
